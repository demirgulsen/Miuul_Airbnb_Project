...
def amsterdam_data_prep(awd_df):
    ...
    binary_cols = [col for col in awd_df.columns if awd_df[col].dtype not in [int, float] and awd_df[col].nunique() == 2]
    ...
    categories_to_replace = [
        ...
        "Private room in villa" , "Private room" , "Room in hostel", "Entire cabin" , "Tiny home" , "Room in serviced apartment",
        "Private room in farm stay", "Entire cottage" ,"Private room in earthen home", "Shared room in houseboat" ,"Casa particular" ,
    ...
    model_df = awd_df[['host_is_superhost', 'accommodates', 'bathrooms', 'bedrooms', 'beds',
                       ...
                       'number_of_reviews', 'review_scores_value', 'has_private_entrance', 'has_self_checkin', 'has_kitchen',
                       'has_bathtub','has_host_greeting', 'has_dishwasher', 'has_longterm', 'has_fireplace', 'has_parking',
...
def main():
    amsterdam_list = pd.read_csv("datasets/all_listings.csv")
